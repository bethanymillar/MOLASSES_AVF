...
VENTS_FILE="./inputs/vent_source/vents_SouthW_H.utm"  # Assuming you have a specific vents file for Auckland
OUTPUT_DIR="./flow_outputs/H/flowH_South"
CSV_CLEANER_SCRIPT="./scripts/convert_to_csv_H.py"
RASTER_CONVERTER_SCRIPT="./scripts/lava_flow_rasterised_H.py"

# Path to the virtual environment directory
VENV_DIR=./molasses_env_H
...
while IFS= read -r line; do
  ...
  echo "$X $Y" > ./inputs/vents_H.utm

  # Run the MOLASSES model and check if it produces an output
  ./bin/molasses_H.ljc ./inputs/molasses_H_SW.conf

  # Check if the flowH0 output file exists
  if [ -f "flowH0" ]; then
    # Define CSV output path
    output_csv="$OUTPUT_DIR/flowH_${X}_${Y}.csv"

    # Run the CSV cleaner script to create a clean CSV
    python3 "$CSV_CLEANER_SCRIPT" "flowH0" "$output_csv"
    ...
    echo "Error: flowH0 file not found for vent at X:$X, Y:$Y"
  fi
