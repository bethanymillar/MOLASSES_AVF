...
VENTS_FILE="./inputs/vent_source/vents_SouthW_D.utm"  # Assuming you have a specific vents file for Auckland
OUTPUT_DIR="./flow_outputs/D/flowD_SouthW"
CSV_CLEANER_SCRIPT="./scripts/convert_to_csv_D.py"
RASTER_CONVERTER_SCRIPT="./scripts/lava_flow_rasterised_D.py"

# Path to the virtual environment directory
VENV_DIR=./molasses_env_D
...
while IFS= read -r line; do
  ...
  echo "$X $Y" > ./inputs/vents_D.utm

  # Run the MOLASSES model and check if it produces an output
  ./bin/molasses_D.ljc ./inputs/molasses_D_SW.conf

  # Check if the flowD0 output file exists
  if [ -f "flowD0" ]; then
    # Define CSV output path
    output_csv="$OUTPUT_DIR/flowD_${X}_${Y}.csv"

    # Run the CSV cleaner script to create a clean CSV
    python3 "$CSV_CLEANER_SCRIPT" "flowD0" "$output_csv"
    ...
    echo "Error: flowD0 file not found for vent at X:$X, Y:$Y"
  fi
